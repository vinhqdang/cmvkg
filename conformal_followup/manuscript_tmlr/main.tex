...
Image 518 of the POPE benchmark is an office desk. On it, in plain view and occupying a
respectable fraction of the frame, is a computer mouse. We ask LLaVA-1.5-7B: \emph{``Is
...
correctness score for this item is $s=0.18$, in the bottom fifth of the distribution. An
...
emit, at most $10\%$ are wrong, with $95\%$ confidence, and this must hold for the next
...
lever: a threshold on a nonconformity score, calibrated on held-out data, above which we
...
\text{abstain}\}$, and this paper is about what happens when a fourth action ---
...
penalise the attention patterns associated with over-trusting the language prior
...
surgically \citep{attnlens,only,mcallava,maskcd,causalgating}; injects the output of a
...
the result is \emph{selective prediction}: emit when a nonconformity score clears a
...
repairs. The obvious composition, \emph{certified repair}, is what our own earlier work
\citep{cmvkgguard} gestured at without achieving: a verify-and-correct pipeline whose
correctness rests on benchmark averages. This paper is, in one sense, the question that
work left open --- what would it take to attach a real guarantee to a system that changes
its own answers?
...
\]
of inputs. The bound does not depend on the quality of the nonconformity score, the
...
repaired buys $1.9$--$4.7$ percentage points of certified coverage, a $3$--$6\times$
multiplier. The mechanism is not ``we fixed some answers''; it is ``fixing a few answers
...
\paragraph{The cheap version is provably impossible.}
...
flagged region equals the model's \emph{accuracy} on that region, so certifying repair at
level $\alpha$ requires exhibiting a region where the model is right less than $\alpha$ of
the time --- a strictly stronger demand than finding a region where it is unconfident.
Empirically the bottom $2\%$ of our score still retains $13.3\%$ accuracy against
$\alpha=0.10$, and the resulting system repairs essentially nothing
(Theorem~\ref{thm:independence}). Certified correction \emph{requires} a second,
independent evidence channel. This is a constraint on system architecture, and it is the
main structural claim of the paper.
...
reversal. We chased the reversal and found it is predictable: the achievable gain is
capped by the abstained mass, so a benchmark with little headroom above $\alpha$ has
little to gain and the same exposure to loss. This yields a checkable precondition
($\mu$ comfortably above $\alpha$, with an applicable evidence channel) that correctly
retrodicts the failure on AMBER-discriminative before the experiment is run
(\S\ref{sec:precondition}). Upside is bounded; downside is not.
...
nonconformity score --- a boring combination of logit and grounding features --- and
...
hallucinated mentions downstream ($n=121$, $p=0.025$). The fixed-point route is closed. We
...
score that flagged the original is circular, for the reason given above. Certified
correction therefore requires a second, independent evidence channel --- a constraint on
system architecture, not a modelling preference.
...
precondition that correctly predicts the one benchmark where CCRC loses.
...
because it determines how informative the model's own logits are as a nonconformity score.
...
and relation types as well as existence. MMHalBench \citep{mmhal} evaluates diverse
hallucination types in question answering using a strong judge model. MME \citep{mme}
...
with no retraining. OPERA \citep{opera} identifies an ``attention sink'' pattern
characteristic of over-trust in the language prior, penalises it in the beam score, and
retrospectively reallocates attention. Visual contrastive decoding \citep{vcd} contrasts
...
component driven by the prior. Instruction-contrastive and focal-contrast variants use
other perturbations of the input \citep{chen2024halc}, and \citet{favero2024} formalise the
contrast as an information-grounding objective. DeGF \citep{degf} closes a generative loop:
...
\citep{attnlens,only}, and interventions range from masking \citep{maskcd} through
layer-selective contrastive readout \citep{mcallava} to causal gating of the information
paths implicated \citep{causalgating}. These give the most detailed available picture of
...
report is a benchmark average after rewriting. Our own prior work \citep{cmvkgguard} is of
exactly this type, and the present paper began as an attempt to attach a guarantee to it.
What we found is that the attachment is not a matter of wrapping the pipeline in a
calibration loop: the natural way to certify a correction is circular
(Theorem~\ref{thm:independence}), and the honest version requires an architectural
...
Theorem~\ref{thm:independence} shows to be fatal for certifying a correction --- a score
...
\citet{crcimposs} ask when conformal risk control can certify structured LLM outputs at
all. They prove the abstention floor $(\mu-\alpha)/(1-\alpha)$ that our whole design is a
response to, establish a hierarchy of bound tightness (Hoeffding $\subseteq$ empirical
Bernstein $\subseteq$ e-value CRC), and validate adaptive conformal inference under
distribution shift. They do not consider editing the output; the emit-or-abstain action
space is an assumption of the theorem rather than a target of the analysis. Our
contribution stands in a specific relation to theirs: we do not weaken or contradict their
bound --- it is correct, and we re-derive it in our notation in
...
that substitution imposes (Theorems~\ref{thm:independence} and~\ref{thm:coupling}).
...
skims both papers may reasonably ask what is left. Their observation is the same as ours:
conformal filtering on VLMs abstains far too much to be useful. Their remedy is to
\emph{acquire more evidence} for borderline claims --- zoomed crops of the relevant image
region --- form a post-acquisition score, and re-threshold, restoring validity by
calibrating on post-acquisition scores rather than pre-acquisition ones. Their setting
(POPE/COCO, LLaVA and Qwen backbones), their machinery (Clopper--Pearson $p$-values with
fixed-sequence testing), and their motivation coincide with ours, and they report certified
coverage rising from $28\%$ to $37\%$ at $\alpha=0.10$.

The distinction is precise. BCEA changes \emph{how confident it is} about the model's
claim; we change \emph{what claim is emitted}. In their own framing there is no corrected
answer: an item that survives acquisition is emitted as the model wrote it, and an item
that does not is abstained. This means BCEA remains inside the premise of
\citet{crcimposs} --- it moves items across the threshold by improving the score, which the
bound permits, and it is therefore still subject to the floor. CCRC leaves the premise.
...
(ii) the proof that such substitutions cannot be certified from the model's own score, so
that a second independent channel is a requirement rather than a design choice, (iii) an
