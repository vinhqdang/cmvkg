...
and we decompose our total margin over a published CLIP-based scorer to show that $66.0$ of
$68.1$ points come from the score and $2.1$ from our algorithm --- the $2.1$ is the number we
...
a genuine \emph{dilution} bonus that fires only on the $12$--$28\%$ of splits where the
...
$66.0$ points to the correctness score and $2.1$ to our own mechanism, and why we report the
$2.1$.
...
\emph{unchanged} on $72$--$88\%$ of splits. The dilution term proper is worth $+0.2$ to
...
Compared against a published conformal VLM scorer we gain $68.1$ points of certified coverage
on one setting. Decomposed, $66.0$ of those points come from the correctness score --- a boring
combination of logit and grounding features --- and $2.1$ from the certified-correction
mechanism itself (\S\ref{sec:attribution}). We would rather a reader trust the $2.1$ than be
sold the $68$.
...
automatically, the calibrated threshold is unchanged on $72$--$88\%$ of splits, and the
...
our margin over a published scorer and report that $66.0$ of $68.1$ points are the score and
$2.1$ the procedure. We reproduce the closest concurrent method \citep{bcea} and find the two
...
$66.0$ points from the correctness score against $2.1$ from certified correction
...
Two pieces of evidence bound the exposure. First, a synthetic study in which the true risk is
known: over four score shapes (well-separated logistic, weakly separated, heavy-tailed, and a
deliberately miscalibrated monotone transform) $\times$ four calibration sizes
($100$, $250$, $500$, $1500$), the realised exceedance $\Pr[\Risk>\alpha]$ of the rank-indexed
procedure is $2.0$--$6.1\%$ against $\delta=0.10$ --- conservative in every cell, and nowhere
near the nominal level. Second, on real data the population-risk exceedance is $0.0$--$5.1\%$
across all $40$ cells of \S\ref{sec:exp} (Table~\ref{tab:audit}). We therefore claim the
...
($59.0\%$ vs.\ $68.2\%$). Conformal coverage depends on how well a score \emph{separates}
...
grounding improves AURC from $0.1039$ to $0.0570$ and certified coverage from $0.4\%$ to
$45.0\%$ ($\Delta\text{AURC}=0.043\pm0.007$).
...
\midrule
POPE-1500 LLaVA    & 81.9\% & 83.1\% & 68.2\% & \textbf{72.6\%} & 59.0\%\\
POPE-adv LLaVA     & 82.7\% & 82.4\% & 42.1\% & \textbf{46.8\%} & 21.3\%\\
POPE-adv Qwen2-VL  & 87.1\% & 82.9\% & 77.5\% & \textbf{79.4\%} & 21.0\%\\
POPE-adv LLaVA+VCD & 80.4\% & 82.9\% & 45.0\% & \textbf{47.5\%} & 21.0\%\\
AMBER(d) LLaVA     & 88.6\% & 95.6\% & \textbf{92.1\%} & 84.5\% & 17.9\%\rlap{$^{\dagger}$}\\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]\footnotesize
\item[$\dagger$] The detector is $95.6\%$ accurate on AMBER and would certify nearly
everything given enough calibration data, but with $n=228$ a three-way split leaves $76$
calibration items and the fixed sequence aborts on $82\%$ of splits (a single error at the
grid start already breaks the Clopper--Pearson bound at $k\approx33$). The figure therefore
measures sample size, not detector quality; we report it rather than a
large-calibration number because it is what this protocol yields. An earlier draft of this
paper reported $92.6\%$ here and claimed a detector-only baseline beats CCRC on AMBER; that
number is not reproducible under any split and the claim is withdrawn.
\end{tablenotes}
...
certifies $71.6\%$ coverage, the betting bound $61.9\%$, empirical Bernstein $24.8\%$, and Hoeffding $21.6\%$
...
\midrule
Hoeffding                & 116 & 21.6\% & 4.1\% & $0\%$ & 75.1\% & 9.7\%  & $0\%$\\
Empirical Bernstein      &  71 & 24.8\% & 3.4\% & $0\%$ & 70.2\% & 9.4\%  & $0\%$\\
Betting (e-CRC)          &  44 & 61.9\% & 6.2\% & $0\%$ & 79.0\% & 10.4\% & $0\%$\\
Clopper--Pearson (exact) &  22 & \textbf{71.6\%} & 8.2\% & $2\%$ & \textbf{86.1\%} & 12.5\% & $5\%$\\
\bottomrule
\end{tabular}
\end{table}
