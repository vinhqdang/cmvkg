...
Figure~\ref{fig:sensitivity} shows accuracy across thresholds $\tau \in [0.50, 0.90]$. The system performs consistently (88--100\%) in the range $[0.50, 0.65]$, with the default $\tau=0.70$ yielding 88.8\%. Accuracy degrades only at $\tau > 0.75$, where the detector becomes over-cautious. This confirms the framework is stable within the recommended operating range and not critically sensitive to small threshold perturbations.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/results_sensitivity.png}
    \caption{Accuracy vs.\ detection threshold $\tau$. Performance is robust in $[0.50, 0.70]$.}
...
Figure~\ref{fig:failure_dist} summarises the failure distribution. We report these failure modes to provide honest evaluation; future work will address wrong-correction with beam-reranking using semantic sentence embeddings.
...
\begin{enumerate}
    ...
    \item \textbf{Known Limitations:} Over-correction of valid tokens absent from the graph, and wrong-correction when graph nodes are contextually mismatched, represent failure modes that motivate future improvements in corrector re-ranking.
\end{enumerate}
