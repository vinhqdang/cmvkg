...
\end{equation}
Combined with CLIP-based semantic similarity, the visual score is:
\begin{equation}
S_{vis}(t_i) = \lambda_1 \text{VAR}(t_i) + \lambda_2 \frac{\mathbf{e}_{t_i} \cdot \mathbf{e}_{\text{img}}}{\|\mathbf{e}_{t_i}\| \|\mathbf{e}_{\text{img}}\|}
\end{equation}
...
Beyond factual correctness, we verify logical consistency using a structural entailment approach. Given a candidate token $t_i$ representing an action or state, we evaluate its validity using the context representations in $\mathcal{G}$:
\begin{equation}
...
\end{equation}
To strictly preserve the training-free nature of the framework, the weight vector $\mathbf{w}$ is not learned via backpropagation. Instead, it is heuristically adapted dynamically based on the softmax distribution of the query-type embeddings (e.g., shifting weight to $S_{vis}$ for structural queries). Therefore, the entire CMVE is a non-differentiable, heuristic verification layer structurally perfect for plug-and-play middleware integration.
...
We define a dynamic threshold $\tau_{dyn}$ based on the local entropy of the generation distribution $\mathcal{H}(P_i)$:
\begin{equation}
\tau_{dyn}(t_i) = \mu + \sigma \cdot \tanh(\mathcal{H}(P(t_i | t_{<i})) - \mathcal{H}_0)
\end{equation}

If $\Omega(t_i) < \tau_{dyn}(t_i)$, the token is flagged as a hallucination. The correction process involves retrieving top-$K$ candidates $\mathcal{C}_{cand}$ from $\mathcal{G}$ and re-ranking them using a contrastive objective:
\begin{equation}
c^* = \arg\max_{c \in \mathcal{C}_{cand}} \left( (1-\gamma) \log P_{\text{VLM}}(c) + \gamma \log P_{\text{KG}}(c | \mathcal{G}) \right)
\end{equation}
where $P_{\text{KG}}$ is the likelihood derived from graph connectivity.

In practical implementations (e.g., wrapping LLaVA), token-level interception is achieved by injecting custom stopping criteria into the autoregressive loop. Upon detecting a hallucination, logits for the flagged token are suppressed, and the VLM's context window is simply updated with the corrected candidate sequence $c^*$, effectively teacher-forcing the grounded reasoning path without any modifications to the underlying VLM source code.
