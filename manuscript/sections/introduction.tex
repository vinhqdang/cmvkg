...
\begin{itemize}
    \item We introduce \textbf{CMVKG-Guard}, an effective training-free, end-to-end middleware that intercepts VLM token generation in real-time. The core novelty lies in the \textit{compositional architecture}: unifying visual, factual, and reasoning verification signals into a single Unified Verification Score (UVS) at the token level during decoding --- something no prior method does simultaneously without retraining.
    ...
    \item We demonstrate through extensive experiments on standard benchmarks (POPE, CHAIR, MMHalBench) that CMVKG-Guard achieves highly competitive performance, reducing object hallucinations by 84\% and achieving >90\% accuracy on the POPE benchmark. We additionally provide sensitivity, robustness, failure case, and Layer 3 independent validation analyses to support scientific rigour.
\end{itemize}
