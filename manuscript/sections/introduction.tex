...
\begin{itemize}
    \item We introduce \textbf{CMVKG-Guard}, an effective training-free, end-to-end system capable of simultaneous real-time hallucination detection and correction without model retraining.
    \item We propose the \textbf{Dynamic Hierarchical Multimodal Knowledge Graph (DH-MMKG)}, a novel data structure that self-constructs during inference to provide context-aware grounding.
    \item We develop a \textbf{Multi-Layered Verification Strategy} that addresses perception, knowledge, and reasoning hallucinations in a unified framework.
    \item We demonstrate through extensive experiments on standard benchmarks (POPE, CHAIR, MMHalBench) that CMVKG-Guard achieves highly competitive performance, reducing object hallucinations by 84\% and achieving >90\% accuracy on the POPE benchmark, with a minimal latency overhead suitable for real-time applications.
\end{itemize}
